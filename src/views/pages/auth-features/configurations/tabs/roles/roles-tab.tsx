...
import RoleDialog from "@/views/pages/auth-features/configurations/tabs/roles/roles-dialog";
...
export default function RolesTab(props: RolesTabProps) {
  ...
  const searchOnChange = (e: any) => {
    ...
    // 1. Filter
    const filtered = roles.unFilterList.filter((item: BasicModel) =>
  ...
  const add = (role: BasicModel) => {
    setRoles((prev) => ({
      unFilterList: [role, ...prev.unFilterList],
      filterList: [role, ...prev.filterList],
    }));
  };
  const edit = (role: BasicModel) => {
    setRoles((prev) => {
      const updatedList = [...prev.unFilterList]; // clone the array
      const index = updatedList.findIndex((item) => item.id === role.id);

      if (index !== -1) {
        updatedList[index] = role; // replace at same position
      }
  ...
  const remove = async (role: BasicModel) => {
    try {
      // 1. Remove from backend
      const response = await axiosClient.delete(`roles/${role.id}`);
      if (response.status === 200) {
        ...
        setRoles((prevJobs) => ({
          ...
          filterList: prevJobs.filterList.filter((item) => item.id !== role.id),
        }));
          ...
          useMemo(
            () => (
              ...
              >
                <RoleDialog
                  role={onEdit}
                  onComplete={(role: BasicModel, isEdited: boolean) =>
                    isEdited ? edit(role) : add(role)
                  }
      ...
      <Table className="bg-card rounded-md mt-1 py-8 w-full">
        ...
        <TableBody className="rtl:text-xl-rtl ltr:text-lg-ltr">
          {loading ? (
            ...
            roles.filterList.map((role: BasicModel, index: number) => (
              <TableRowIcon
                read={hasView}
                remove={hasDelete}
                edit={hasEdit}
                onEdit={async (role: BasicModel) => setOnEdit(role)}
                key={index}
                item={role}
                onRemove={remove}
                onRead={async () => {}}
              >
                ...
                <TableCell>
                  {toLocaleDate(new Date(role.created_at), state)}
                </TableCell>
              </TableRowIcon>
            ))
          )}
